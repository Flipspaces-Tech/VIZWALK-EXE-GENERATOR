  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const match =
          items.find((x) => norm(x.buildName) === q) ||
          items.find((x) => norm(x.projectName) === q) ||
          items[0];
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        url.searchParams.set("build", effectiveBuild);
...
const dl = (url) => {
  ...
  const version = headerItem?.buildVersion || "V1";
  const projectSlot =
